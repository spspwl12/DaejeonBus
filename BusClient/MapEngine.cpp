...
BOOL LoadSeek(LPCSTR path)
{
	...
	if( hFile = CreateFile(path, GENERIC_READ, 0, 0, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0) )
	{
		...
		while(Pos < dwFileSize)
		{
			...
			if (nBytesRead != sizeof(tmp))
				memset(tmp + nBytesRead, 0, sizeof(tmp) - nBytesRead);

			St = tmp;
			if (!(rTn = strchr(tmp, 10)))
				rTn = tmp + nBytesRead;
			Ed = strchr(St, ' ');
			Indx = 0;

			while (Ed && St < rTn && Indx >= 0)
			{
				*Ed = 0;
		...
		if (Index_X >= MapRect.left && Index_X <= MapRect.right && Index_Y >= MapRect.top && Index_Y <= MapRect.bottom)
		{
			...
			switch (__DrawMarkup[j].Type)
			{
				...
				case 4:
				{
					...
					HBITMAP hOldBitmap2 = (HBITMAP)SelectObject(hMemDC2, (HBITMAP)__DrawMarkup[j].hBitmap);

					TransparentBlt(hMemDC, _x - 16, _y - 16, 32, 32, hMemDC2, __DrawMarkup[j].xSrc, __DrawMarkup[j].ySrc, 32, 32, 0xFF00FF);
...
VOID SetMarkImage(HBITMAP hBitmap, double _lat, double _long, int xSrc, int ySrc)
{
	__DrawMarkup[__DrawMarkupCnt].Type = 4;
	__DrawMarkup[__DrawMarkupCnt].lat = _lat;
	__DrawMarkup[__DrawMarkupCnt].lon = _long;
	__DrawMarkup[__DrawMarkupCnt].hBitmap = hBitmap;
	__DrawMarkup[__DrawMarkupCnt].xSrc = xSrc;
	__DrawMarkup[__DrawMarkupCnt].ySrc = ySrc;
	__DrawMarkup[__DrawMarkupCnt].pMemo = NULL;
